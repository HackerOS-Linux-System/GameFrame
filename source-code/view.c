...
char *
view_get_title(struct gf_view *view)
{
...
bool
view_is_primary(struct gf_view *view)
{
	return view->impl->is_primary(view);
}

bool
view_is_transient_for(struct gf_view *child, struct gf_view *parent)
{
	return child->impl->is_transient_for(child, parent);
}

void
view_activate(struct gf_view *view, bool activate)
{
...
static bool
view_extends_output_layout(struct gf_view *view, struct wlr_box *layout_box)
{
	int width, height;
...
static void
view_maximize(struct gf_view *view, struct wlr_box *layout_box)
{
...
static void
view_center(struct gf_view *view, struct wlr_box *layout_box)
{
	int width, height;
	view->impl->get_geometry(view, &width, &height);
...
void
view_position(struct gf_view *view)
{
...
void
view_position_all(struct gf_server *server)
{
	struct gf_view *view;
...
void
view_unmap(struct gf_view *view)
{
...
{
	struct gf_view *view = wl_container_of(listener, view, request_activate);
...
{
	struct gf_view *view = wl_container_of(listener, view, request_close);
	view->impl->close(view);
}

void
view_map(struct gf_view *view, struct wlr_surface *surface)
{
...
#if GAMEFRAME_HAS_XWAYLAND
	if (view->type != GAMEFRAME_XWAYLAND_VIEW || xwayland_view_should_manage(view))
#endif
	{
		view_position(view);
	}
	...
	// For scaling if game resolution differs from output
	if (view->server->game_width > 0 && view->server->game_height > 0 && (view->server->game_width != view->server->nested_width || view->server->game_height != view->server->nested_height)) {
		view->lx = 0;
		view->ly = 0;
		wlr_scene_node_set_position(&view->scene_tree->node, 0, 0);
		struct wlr_scene_node *buffer_node = view->scene_tree->node.children.next;
		struct wlr_scene_buffer *scene_buffer = wlr_scene_buffer_from_node(buffer_node);
		wlr_scene_buffer_set_dest_size(scene_buffer, view->server->nested_width, view->server->nested_height);
	}
}

void
view_destroy(struct gf_view *view)
{
	...
	if (!empty) {
		struct gf_view *prev = wl_container_of(server->views.next, prev, link);
		seat_set_focus(server->seat, prev);
	}
}

void
view_init(struct gf_view *view, struct gf_server *server, enum gf_view_type type, const struct gf_view_impl *impl)
{
	view->server = server;
	view->type = type;
	view->impl = impl;
